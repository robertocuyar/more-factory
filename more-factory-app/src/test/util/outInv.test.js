...
const testMachine = defaultMachine;
const testInput = testMachine.machines[0].input[1];
...
test("outInv returns inventory and machines that is not changed if requested item is not present in inventory",()=>{
   expect(outInv(testInput, defaultBag, testMachine).inventory.slots[0].content).toBe(null);
   expect(outInv(testInput, defaultBag, testMachine).machine.machines[0].input[1].numContent).toBe(0);

});

test("outInv returns an object with an inventory and machine property.", ()=>{
    invMachineResult= outInv(testInput, defaultBag, testMachine);
